...
func listAsksFromActorResult(ctx context.Context, plumbing claPlubming, actorResult state.GetAllActorsResult, out chan Ask) error {
	...
	// ret, err := plumbing.MessageQuery(ctx, address.Undef, addr, integration.LegacyMethod("miner.GetAsks"), plumbing.ChainHeadKey())
	// if err != nil {
	// 	return err
	// }
...
func ClientVerifyStorageDeal(ctx context.Context, plumbing cvsdPlumbing, proposalCid cid.Cid, proofInfo *storagedeal.ProofInfo) error {
	...
	// _, err = plumbing.MessageQuery(ctx, address.Undef, integration.LegacyMethod("deal.Miner"), integration.LegacyMethod("miner.VerifyPieceInclusion"), plumbing.ChainHeadKey(), params...)
	// if err != nil {
	// 	return err
	// }
...
func getAskByID(ctx context.Context, plumbing claPlubming, addr address.Address, id uint64) (Ask, error) {
	...
	// _, err := plumbing.MessageQuery(ctx, address.Undef, addr, integration.LegacyMethod("miner.GetAsk"), plumbing.ChainHeadKey(), big.NewInt(int64(id)))
	// if err != nil {
	// 	return Ask{}, err
	// }
